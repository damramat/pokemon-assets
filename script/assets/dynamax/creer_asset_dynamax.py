...
INPUT_DIR = "../Addressable Assets/"
OUTPUT_DIR = "result_dynamax/"
LOGO_PATH = "dynamax_logo.png"
...
def is_allowed_file(filename: str) -> bool:
    ...
    # EXCLUSION : tout fichier contenant 'GIGANTAMAX'
    if "gigantamax" in filename.lower():
        return False

    # Exclusion par suffixes
...
def generate_dynamax(pokemon_path: str, output_path: str, logo: Image.Image):
    """Génère l'image Dynamax pour un fichier donné."""
...
def main():
    ...
    print("Génération des assets Dynamax…\n")

    for filename in files:
        in_path = os.path.join(INPUT_DIR, filename)

        # Nom de sortie ex : pm1.icon_dynamax.png
        base, ext = os.path.splitext(filename)
        out_name = f"{base}_dynamax{ext}"
    ...
    print("\nTerminé ! Les images Dynamax sont dans :", OUTPUT_DIR)
